...
export default function Admin_review_assignment() {
  ...
  // ✅ Only include submitted, accepted, or published items
  const processingItems = filtered.filter((thesis) =>
    !["submitted", "accepted", "published", "rejected"].includes(
      thesis.status?.toLowerCase()
    )
  );

  const totalPages = Math.ceil(processingItems.length / itemsPerPage);
  const currentItems = processingItems.slice(
