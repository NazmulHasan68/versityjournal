    ...
    <div className="w-full min-h-[550px] md:min-h-screen bg-white p-4 font-sans flex flex-col justify-between">
      <div>
        <div className="mb-8">
          ...
          <p className="text-xs text-gray-500 mt-1">Manage your reviews and messages</p>
        </div>

        <nav className="space-y-1">
          {reviewerLinks.map(({ path, label, icon }) => (
            <SidebarLink
              key={path}
              path={path}
              label={label}
  ...
  return (
    <>
      ...
      <div className="md:hidden p-4 border-b flex items-center justify-between bg-white z-50">
        <h2 className="text-lg font-semibold text-blue-700">Reviewer Panel</h2>
